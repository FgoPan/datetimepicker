<template>
  <div class="datepicker">
    <input class="form-control datepicker-input" :class="{'with-reset-button': clearButton}" type="text" :placeholder="placeholder" readonly
        :style="{width:width}"
        @click="inputClick"
        v-model="value"/>
      ...
      <div class="datepicker-inner">
        <div class="datepicker-body">
          ...
          <div class="datepicker-timeRange">
            ...
            <button style="margin-bottom: 4px;" class="pull-right btn btn-primary" type="button" @click="setTime">确定</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    setTime () {
      this.value = this.stringify(this.currDate) + " " + this.hour + ":" + this.minute
...
.datepicker-timeRange .d ul li label:hover{
  background-color: #eeeeee;
}
